...
def find_closest(location, centroids):
    ...
    minimum_distance = distance(location, centroids[0])
    for centroid in centroids:
        if distance(centroid, location) < minimum_distance:
            minimum_distance = distance(centroid, location)
...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        rest_location = restaurant_location(restaurant)
        closest = find_closest(rest_location, centroids)
        pairs.append([closest, restaurant])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    highest_rsqr = 0
    best_predictor_fn = None
    for i in range(0, len(feature_fns)):
        feature = feature_fns[i]
        predictor, r_squared = find_predictor(user, reviewed_restaurants, feature)
        if r_squared > highest_rsqr:
            highest_rsqr = r_squared
            best_predictor_fn = predictor
    return best_predictor_fn
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed_restaurants:
            rating_dictionary[name] = user_rating(user,name)
        else:
            rating_dictionary[name] = predictor(restaurant)
